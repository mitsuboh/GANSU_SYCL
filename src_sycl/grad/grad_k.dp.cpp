...
SYCL_EXTERNAL
void compute_gradients_kinetic(const sycl::nd_item<1>& item_ct1, double* g_gradients, const real_t* g_density_matrix, const PrimitiveShell* g_shell, const real_t* g_cgto_normalization_factors, 
                                const int num_basis, ShellTypeInfo shell_s0, ShellTypeInfo shell_s1, const size_t num_threads)
{
    ...
    AddToResult(g_gradients, 3*a.atom_index+0, result_Ax, i != j);
    AddToResult(g_gradients, 3*a.atom_index+1, result_Ay, i != j);
    AddToResult(g_gradients, 3*a.atom_index+2, result_Az, i != j);

    AddToResult(g_gradients, 3*b.atom_index+0, -result_Ax, i != j);
    AddToResult(g_gradients, 3*b.atom_index+1, -result_Ay, i != j);
    AddToResult(g_gradients, 3*b.atom_index+2, -result_Az, i != j);
}
