...
#include "device_function_grad_g_s.txt"

// ---- compute_gradients_two_electron ----
SYCL_EXTERNAL
void compute_gradients_two_electron(const sycl::nd_item<1>& item_ct1, double* g_gradients, const real_t* g_density_matrix, const PrimitiveShell* g_shell, const real_t* g_cgto_normalization_factors,
                                    ...
                                    const size_t num_threads, const int num_basis, const double* g_boys_grid)
{
    ...
    for(int lmn_a=0; lmn_a<comb_max(a.shell_type); lmn_a++){
        auto A_ang = make_int3(loop_to_ang[a.shell_type][lmn_a][0], loop_to_ang[a.shell_type][lmn_a][1], loop_to_ang[a.shell_type][lmn_a][2]);
        Norm_A = calcNorm(alpha, A_ang.x, A_ang.y, A_ang.z);

        for(int lmn_b=0; lmn_b<comb_max(b.shell_type); lmn_b++){
            auto B_ang = make_int3(loop_to_ang[b.shell_type][lmn_b][0], loop_to_ang[b.shell_type][lmn_b][1], loop_to_ang[b.shell_type][lmn_b][2]);
            Norm_B = calcNorm(beta, B_ang.x, B_ang.y, B_ang.z);

            for(int lmn_c=0; lmn_c<comb_max(c.shell_type); lmn_c++){
                auto C_ang = make_int3(loop_to_ang[c.shell_type][lmn_c][0], loop_to_ang[c.shell_type][lmn_c][1], loop_to_ang[c.shell_type][lmn_c][2]);
                Norm_C = calcNorm(gamma, C_ang.x, C_ang.y, C_ang.z);

                for(int lmn_d=0; lmn_d<comb_max(d.shell_type); lmn_d++){
                    auto D_ang = make_int3(loop_to_ang[d.shell_type][lmn_d][0], loop_to_ang[d.shell_type][lmn_d][1], loop_to_ang[d.shell_type][lmn_d][2]);
