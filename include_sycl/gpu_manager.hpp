...
void computeFockMatrix_Direct_RHF(const real_t* d_density_matrix, const real_t* d_core_hamiltonian_matrix, const std::vector<ShellTypeInfo>& shell_type_infos, const std::vector<ShellPairTypeInfo>& shell_pair_type_infos, const PrimitiveShell* d_primitive_shells, const sycl::int2* d_primitive_shell_pair_indices, const real_t* d_cgto_nomalization_factors, const real_t* d_boys_grid, const real_t* d_schwarz_upper_bound_factors, const real_t  schwarz_screening_threshold, real_t* d_fock_matrix, const int num_basis, std::vector<int*>& d_global_counters, std::vector<int*>& d_min_skipped_columns, real_t* d_fock_matrix_replicas, const int num_fock_replicas, const int verbose);
